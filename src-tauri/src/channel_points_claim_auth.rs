...
#[derive(Debug, Error)]
pub enum ChannelPointsClaimAuthError {
    #[error("{0}")]
    Message(String),
    #[error(transparent)]
...
#[derive(Debug, Clone)]
pub(crate) struct ChannelPointsClaimAuthSession {
    pub token: String,
    pub user_id: String,
}
...
#[serde(rename_all = "camelCase")]
pub struct ChannelPointsClaimAuthStatus {
    pub configured: bool,
...
pub fn get_status() -> Result<ChannelPointsClaimAuthStatus, ChannelPointsClaimAuthError> {
    let auth = load_auth()?;
    Ok(ChannelPointsClaimAuthStatus {
...
) -> Result<Option<ChannelPointsClaimAuthSession>, ChannelPointsClaimAuthError> {
    Ok(load_auth()?.map(|auth| ChannelPointsClaimAuthSession {
        token: auth.token,
        user_id: auth.user_id,
    }))
}

pub async fn start_device_flow() -> Result<TvDeviceCodeResponse, ChannelPointsClaimAuthError> {
    ...
    if !status.is_success() {
        return Err(ChannelPointsClaimAuthError::Message(format!(
            "Bonus-claim TV device login failed (HTTP {status})"
        )));
    }
...
async fn validate_token(token: &str) -> Result<ValidateBody, ChannelPointsClaimAuthError> {
    ...
    if !status.is_success() {
        return Err(ChannelPointsClaimAuthError::Message(format!(
            "Twitch rejected the bonus-claim TV token (HTTP {status})"
        )));
    }
    let validate = response.json::<ValidateBody>().await?;
    if validate.client_id != TV_CLIENT_ID {
        return Err(ChannelPointsClaimAuthError::Message(
            "Twitch returned a bonus-claim token for the wrong client".into(),
        ));
    }
    Ok(validate)
}
...
pub async fn poll_device_token(
    device_code: &str,
) -> Result<TvDevicePoll, ChannelPointsClaimAuthError> {
    ...
    if expected.as_deref() != Some(device_code) {
        return Err(ChannelPointsClaimAuthError::Message(
            "Bonus-claim TV device login is not active".into(),
        ));
    }
    ...
    if !response.status().is_success() {
        ...
        return match error.message.as_deref() {
            ...
            Some("expired_token") | Some("access_denied") => {
                Err(ChannelPointsClaimAuthError::Message(
                    error
                        .message
                        .unwrap_or_else(|| "Bonus-claim TV login failed".into()),
                ))
            }
            _ => Err(ChannelPointsClaimAuthError::Message(format!(
                "Bonus-claim TV token poll failed (HTTP {status})"
            ))),
        };
    }
    ...
    let app_session = crate::auth::get_session()
        .await
        .map_err(|error| ChannelPointsClaimAuthError::Message(error.to_string()))?;
    if !app_session.logged_in || app_session.user_id.as_deref() != Some(validate.user_id.as_str()) {
        return Err(ChannelPointsClaimAuthError::Message(
            "Bonus-claim TV login belongs to a different Twitch account".into(),
        ));
    }
...
pub fn clear() -> Result<ChannelPointsClaimAuthStatus, ChannelPointsClaimAuthError> {
    clear_auth()?;
    if let Ok(mut pending) = pending_device_code().lock() {
        *pending = None;
    }
